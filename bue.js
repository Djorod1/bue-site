...
const B = {
  ...
  galerie: [
    "Soirée de bienvenue L1 — 2025","Tournoi de football inter-filières","Assemblée Générale BUE",
    "Atelier CRISTAL — Python","Gala de fin d'année 2025","Conférence IA & Statistiques",
    "Remise des prix BUE","Activité Club Science Ouverte","Sortie pédagogique ENSPD",
    "Permanence BUE","Réunion bureau exécutif","Activité sportive étudiante",
  ],
};
...
function renderBureau() {
  ...
  el.innerHTML = B.bureau.map(m => `
    <div class="card membre-card" style="position:relative">
...
function actuBueCard(a) {
  return `<div class="card actu-card" onclick="showActuBue(${a.id})" style="cursor:pointer">
...
function eventItem(e) {
  ...
  return `<div class="event-item" onclick="showEvent(${e.id})" style="cursor:pointer">
...
function renderRes(cat = '') {
  ...
  el.innerHTML = list.map(r => `
    <div class="res-item" onclick="toast('Téléchargement : ${r.titre.substring(0,40)}...','ok')" style="cursor:pointer">
...
function renderForumCats() {
  ...
  el.innerHTML = B.forum.cats.map(c => `
    <div class="forum-cat" onclick="filterFils('${c.titre}',this)" style="cursor:pointer">
...
function renderFils(cat = '') {
  ...
  el.innerHTML = list.map(f => `
    <div class="thread" onclick="showFil(${f.id})" style="cursor:pointer">
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Fermer modal en cliquant l'arrière-plan
